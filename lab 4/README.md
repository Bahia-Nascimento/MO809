...
## Resultado oficial de 22/09/2026

A busca atual inclui dropout `0 / 0,1 / 0,2 / 0,3 / 0,4` e Batch Normalization
opcional. Os padrões agora são **24 trials × até 20 épocas**, retreinamento por até
30 épocas e EarlyStopping com paciência 5. Os modelos e resultados oficiais estão
diretamente em [`output/`](output/README.md).
...
### Margem de treinamento e limiar de decisão

São parâmetros diferentes. A **margem** aparece na Contrastive Loss e altera os
gradientes durante o treinamento. O **limiar** é aplicado depois do treinamento para
converter uma distância em `similar` ou `diferente`.

O Keras Tuner pode testar uma margem com `hp.Choice`, mas cada valor exige treinar
uma nova rede. Não devemos escolher o vencedor comparando diretamente `val_loss`,
porque mudar a margem também muda a escala da própria loss. Para tunar a margem de
forma metodologicamente correta, a rede deve produzir embeddings L2-normalizados
(distância limitada entre 0 e 2) e o Tuner deve usar uma métrica comparável entre
trials, como acurácia balanceada ou AUC de validação. Se o limiar for calibrado para
cada trial, essa calibração deve usar uma subdivisão separada dos dados usados para
escolher o trial, evitando selecionar e avaliar no mesmo conjunto.

Nesta entrega mantemos a margem em 1 e calibramos somente o limiar de decisão. Isso
preserva o objetivo do laboratório e permite comparar a Contrastive Loss de todos os
trials na mesma escala.

## Keras Tuner

Usamos `RandomSearch`, objetivo `val_loss`, semente 42, 24 trials de até 20 épocas.
Cada trial começa com pesos próprios. O melhor conjunto de hiperparâmetros é usado
para treinar uma nova rede por até 30 épocas, com `EarlyStopping` (paciência 5) e restauração dos
pesos da melhor época de validação. Comparamos o retreinamento ao melhor checkpoint
da busca e salvamos o de menor `val_loss`, antes de avaliar o teste.
...
| Batch Normalization | Ativada ou desativada nas duas convoluções e camada densa |
...
São 480 combinações possíveis; testar 24 não garante o ótimo global. Batch size 128,
...
```bash
python treinar.py --output output/busca_maior --trials 40 --epocas-busca 30 --epocas 40
```

BN usa estatísticas do lote no treino e médias móveis salvas na inferência.
Clientes sempre usam `training=False`, para uma imagem isolada ter a mesma
representação que teria em um lote. O dropout também fica inativo na inferência.
Por isso, com regularização ativa, a loss de treino do `fit` não é diretamente
comparável à validação; também registramos a loss de treino em modo de inferência.
