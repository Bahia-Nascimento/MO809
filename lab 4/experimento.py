...
def esperar_servidor(process, ready_file, instance_id):
    ...
    while time.monotonic() < limite:
        if process.poll() is not None:
            raise RuntimeError("Servidor terminou antes de concluir a inicialização.")
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="lab4-") as tmp:
        ready_file = Path(tmp) / "ready.json"
        with (Path(tmp) / "servidor.log").open("w") as log:
    ...
    print(f"Resultados estruturados em {out / 'experimento.json'}")
