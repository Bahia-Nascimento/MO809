...
class TestRegularizacao(unittest.TestCase):
    def test_batchnorm_dropout_serializacao_e_inferencia_individual(self):
        # Força a opção com BN para cobrir o caminho mesmo se o Tuner preferir sem BN.
        import keras_tuner as kt
        from modelo import construir_para_tuner
        hp = kt.HyperParameters()
        hp.Fixed("batch_normalization", True)
        hp.Fixed("dropout", 0.3)
        modelo = construir_para_tuner(hp)
        base = modelo.get_layer("rede_base")
        bn = base.get_layer("bn_conv_1")
        rng = np.random.default_rng(123)
        a = rng.random((4,28,28,1), dtype=np.float32)
        b = rng.random((4,28,28,1), dtype=np.float32)
        antes = bn.moving_mean.numpy().copy()
        modelo.train_on_batch([a,b], np.array([[1],[0],[1],[0]], dtype=np.float32))
        self.assertFalse(np.array_equal(antes, bn.moving_mean.numpy()))
        media = bn.moving_mean.numpy().copy()
        junto = base(a, training=False).numpy()
        individuais = np.concatenate([base(a[i:i+1], training=False).numpy() for i in range(4)])
        np.testing.assert_allclose(junto, individuais, atol=1e-5)
        np.testing.assert_array_equal(media, bn.moving_mean.numpy())
        # Checkpoint deve preservar as médias móveis além dos pesos treináveis.
        with tempfile.TemporaryDirectory() as temp:
            path = Path(temp) / "bn.keras"
            modelo.save(path)
            salvo = tf.keras.models.load_model(path, compile=False)
            np.testing.assert_allclose(modelo([a,b], training=False), salvo([a,b], training=False), atol=1e-6)
