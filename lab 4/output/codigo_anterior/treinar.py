...
def main():
    ...
    parser.add_argument("--epocas-busca", type=int, default=4)
    ...
    if min(args.trials, args.epocas_busca, args.epocas, args.batch_size) < 1:
    ...
    tuner = kt.RandomSearch(construir_para_tuner, objective="val_loss",
    ...
    tuner.search(treino, validation_data=validacao, epochs=args.epocas_busca,
                 callbacks=[tf.keras.callbacks.EarlyStopping(monitor="val_loss", patience=2)], verbose=2)
    ...
    ax.set(xlabel="Época", ylabel="Contrastive Loss", title="Treinamento da rede escolhida pelo Keras Tuner")
